...
interface CentralDifferenceTableProps {
  xValues: number[];
  yValues: number[];
...
export default function CentralDifferenceTable({
  xValues,
  yValues,
  centralDifferences,
  centerIndex,
  highlightIndices,
}: CentralDifferenceTableProps) {
  const [visibleRows, setVisibleRows] = useState(0);
  const totalRows = yValues.length;
  const rowRevealDelay = 120;
  const maxOrder = centralDifferences.length - 1;
  const baseHeight = 72;
  ...
  const getDeltaSymbol = (order: number) => {
    const superscripts = ["", "", "²", "³", "⁴", "⁵", "⁶", "⁷", "⁸", "⁹"];
    return order === 0 ? "y" : `Δ${superscripts[order] || order}`;
  };
  ...
  return (
    <div className="overflow-x-auto">
              ...
              >
                ...
                {centralDifferences.slice(1).map((_, order) => {
                  ...
                  if (isPrimaryRow && diffOrder % 2 === 0) {
                    ...
                    value = diffIndex >= 0 ? centralDifferences[diffOrder][diffIndex] : undefined;
                  ...
                  return (
                    <td
                      ...
                      title={value !== undefined ? `Central difference Δ${diffOrder}y` : ""}
                    >
      ...
      <p className="text-xs text-slate-600 mt-2">
        <span className="bg-yellow-100 px-2 py-1 rounded">Center point at index {centerIndex}</span>
      </p>
    </div>
  );
}
