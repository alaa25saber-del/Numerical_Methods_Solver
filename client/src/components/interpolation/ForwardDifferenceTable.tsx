...
interface ForwardDifferenceTableProps {
  xValues: number[];
  yValues: number[];
...
export default function ForwardDifferenceTable({
  xValues,
  yValues,
  forwardDifferences,
  highlightIndices,
}: ForwardDifferenceTableProps) {
  const [visibleRows, setVisibleRows] = useState(0);
  const totalRows = yValues.length;
  const rowRevealDelay = 110;
  const maxOrder = forwardDifferences.length - 1;
  const baseHeight = 72;
      ...
      <table className="min-w-full border-collapse text-sm rounded-2xl border border-slate-300 bg-white">
        <thead>
          <tr className="bg-slate-100 border-b border-slate-300">
            ...
            {forwardDifferences.slice(1).map((_, order) => (
              ...
              >
                ...
                {forwardDifferences.slice(1).map((_, order) => {
                  ...
                  if (isPrimaryRow && diffOrder % 2 === 0) {
                    ...
                    value = diffIndex >= 0 ? forwardDifferences[diffOrder][diffIndex] : undefined;
                  ...
                  return (
                    <td
                      ...
                      title={value !== undefined ? `Forward difference value Δ${diffOrder}y` : ""}
                    >
